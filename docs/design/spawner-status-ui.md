...
- **핵심 메커니즘**:
  1. World-space `SpawnerCooldownBar` 제거 → 화면 **좌측 하단 2/3 폭**의 `SpawnerStatusPanel` 의 6 셀(134×168) 로 통합 (셀당 색칩+종명+×N+진행 바, 상단 강화 아이콘 row). **v0.6 (64×80) 대비 추가 2.09× 확대** — 사용자의 화면 가로 분할 정책 (좌측 2/3 = 스포너, 우측 1/3 = 빌드) 채택으로 좌측 영역을 가득 채우는 셀 크기 결정 (§7 v0.7 변경 이력).
  ...
  3. `BuildPanel` 은 종 강화 6장 제외(`ECardCategory.Enhance && IsPassive` 필터) + 패널 클릭 시 화면 중앙 `BuildModalPopup` (픽한 모든 카드 표시). **위치는 화면 우측 1/3 폭의 세로 컬럼** (폭 427px, §2.8 v0.7) — SpawnerStatusPanel 과 가로 분할로 시각 경쟁 회피. **PassiveSection / ActiveSection 각각이 `CHPoolingScrollView<BuildIconCell, BuildEntry>` 파생 컴포넌트** (§2.8.5 v0.8) — Rule 11 의 `ScrollRect + 수동 풀링 → CHPoolingScrollView` 원칙 완전 적용으로 풀(패시브 9 + 액티브 10) 모두 픽 시 클리핑/풀링을 단일 컴포넌트가 자동 처리.
...
- **BuildPanel 섹션 CHPoolingScrollView** (§2.8.5 v0.8 — Rule 11 완전 적용, v0.5 의 raw ScrollRect 정책 교체)
- **BuildModalPopup 섹션 CHPoolingScrollView** (§2.7.2 v0.8 — Rule 11 완전 적용, v0.3 의 단순 ScrollRect 예외 철회)
- **SpawnerStatusTooltip 본문 CHPoolingScrollView** (§2.5.5 v0.8 — Rule 11 완전 적용, v0.7 의 raw ScrollRect 정책 교체)
...
[N=1 — ×N 미노출 케이스, 다수]
┌─ 134 × 168 dp ───────────────────────────────────┐
│  ◉H                                              │ ← 아이콘 row (높이 42) — 종에 강화 적용 시 원 1개
├──────────────────────────────────────────────────┤
...
[N≥2 — ×N 노출 케이스, 색칩 일시 숨김 (§2.2.3 v0.5)]
┌─ 134 × 168 dp ───────────────────────────────────┐
│  ◉H×2                                            │ ← 아이콘 row (높이 42) — 원 1개 + ×N 배지
├──────────────────────────────────────────────────┤
│  Wisp                                  ×2        │ ← 종명 + ×N (색칩·gap 일시 숨김 → 종명 가용 폭 76px)
│  ▓▓▓▓▓▓▓░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░        │ ← 진행 바 (높이 17)
│                                                  │ ← 잔여 padding
└──────────────────────────────────────────────────┘
```

> 도식의 `◉H` / `◉H×2` 는 "WispHpBoost 1픽 / 2픽" 예시. 종 1 ↔ 카드 1 매핑이므로 row 의 distinct 아이콘은 항상 0 또는 1 (§2.3 전제). N≥2 케이스에서 색칩이 사라져도 셀 상단 강화 아이콘 row 의 배경색(§2.3.3) 과 월드 디스크 본체 틴트(§2.4) 가 종 색상을 표시 — 인지 손해 없음 (§2.2.3 v0.5 색칩 일시 숨김 결정 사유).
...
셀 상단에 **한 종에 적용된 강화 카드 아이콘 1개** + 중첩 픽 배지를 표시.

**전제 (컨셉 §11.3 종 1 ↔ 강화 카드 1 매핑)**: 한 종에는 그 종 전용 강화 카드가 정확히 1장만 존재한다 (Wisp ↔ WispHpBoost, Wraith ↔ WraithDamageBoost, … Phantom ↔ PhantomMoveSpeedBoost). 따라서 셀이 한 종을 출력 중일 때 **row 의 distinct 아이콘 수는 항상 0 (강화 1장도 안 픽) 또는 1 (해당 종 강화 카드 픽됨)** 이다. 다종 강화가 한 셀에 모이는 케이스는 구조상 발생하지 않는다.

#### 2.3.1 한 개 아이콘 치수 (v0.7)
...
| Row 좌측 정렬 (셀 좌측 padding 기준) | **12px** (셀 padding 과 일치) | 6 → 12 |
| Row 영역 가용 폭 | **110px** (실제 사용은 원 1개 30px + ×N 배지 ~20px = ~50px, 여유 充足) | 52 → 110 |
| 강화 미적용 시 | row 영역 비움 (아이콘 0 개, 빈 **42px** 공간 유지로 본체 영역 위치 고정) | 20 → 42 |

#### 2.3.2 중첩 픽 ×N 배지

| 항목 | 값 | v0.6 비교 |
|---|---|---|
| 표시 조건 | `PickCount ≥ 2` 일 때만 | (유지) |
...
스펙 §6.2 의 H/D/S/R/M/P 매핑을 **그대로 확정**한다.
...
**결정 사유**:
- 원 지름 **14px** (v0.4) 안에 들어가야 함 → 한글 1자(체/력/속/사/이/독)는 **11pt** (v0.4) 에서도 획 굵기 대비 원 면적이 빠듯해 가독성 손해. 영문 1자 유지가 안정적.
- 배경색은 종 색(컨셉 §11.4)을 그대로 사용 — 색칩과 같은 색이라 직관적으로 "어느 종에 붙은 강화"인지 즉시 인지.
- 글자는 영문 약자 — 스탯의 영문 머리글자 1개 (H/D/S/R/M/P 모두 unique). 한국어 직관성보다 글자 가독성과 카드 ID 와의 일관성을 우선.

#### 2.3.4 아이콘 표시 갱신

| 트리거 | 동작 |
|---|---|
| `BattleController.OnTypeModifierChanged(EMonster)` (신규) | 출력 종이 일치하는 모든 셀의 아이콘 row 재구성 |
| `Spawner.OnOutputTypeChanged` (기존) | 해당 셀의 아이콘 row 재구성 (출력 종 자체가 바뀌었으므로 새 종 기준으로) |

같은 종을 출력하는 스포너 셀들은 항상 동일한 아이콘 row 를 가진다 (강화는 종 단위 글로벌).
...
│ ─────────────────────────────── │
│ ┌─ CHPoolingScrollView (BuffLine, AppliedBuff) ┐ │ ← 강화 줄 영역 (v0.8)
│ │ ◉H ×2  체력 ×2.25           │ │ ← BuffLine prefab 1개 (한 줄)
│ │         (200 → 450)         │ │
│ │ (현 페이싱에선 0 또는 1 줄)   │ │
│ └─────────────────────────────┘ │
└─────────────────────────────────┘
...
> 종 1 ↔ 강화 카드 1 매핑이므로 강화 줄은 **현 페이싱에선 0줄 (강화 미적용) 또는 1줄** (해당 종 강화 픽됨). v0.7 에서 강화 줄 영역을 ScrollRect 로 래핑한 이유는 사용자 명시 요청 — **미래 1↔다 매핑 확장 또는 PickCount 누적 표현 풍부화에 대비한 구조 보험**. v0.8 에서 Rule 11 의 `ScrollRect + 수동 풀링 → CHPoolingScrollView` 원칙을 완전 적용해 **`CHPoolingScrollView<BuffLine, AppliedBuff>` 파생 컴포넌트로 재단정** — `SetItemList(IReadOnlyList<AppliedBuff>)` 단일 호출로 줄 수만큼 BuffLine 인스턴스 자동 생성·재사용. 헤더는 CHPoolingScrollView 외부에 고정되어 스크롤 시에도 항상 보임.
...
**CHPoolingScrollView 구조 (v0.8 신규 — Rule 11 완전 적용)**:
...
- BuildPanel 의 루트 GameObject (또는 Background Image) 에 **`CHButton`** 부착 (Rule 11).
...
| 섹션 내부 카드 셀 높이 | 56px |
| 카드 셀 간 간격 | 4px |
| 스크롤 | 각 섹션 내부 **`CHPoolingScrollView<BuildModalCardCell, BuildEntry>`** 파생 컴포넌트 1세트 (v0.8 — v0.3 의 "Rule 11 예외" 결정 철회. 사용자 명시 결정 "룰에 chscrollview 쓰라고 안되어있어?" 로 Rule 11 의 `ScrollRect + 수동 풀링 → CHPoolingScrollView` 원칙 완전 적용) |
...
- 현재 `Assets/_Lair/Scripts/UI/BuildModalPopup.cs:118` 안에 nested 정의되어 있는 `BuildModalCardCell` 클래스를 **별도 파일 `Assets/_Lair/Scripts/UI/BuildModalCardCell.cs` 로 분리**한다 — CHPoolingScrollView 의 TItem 으로서 `_origin` prefab 의 component로 직렬화되어야 함. (Rule 13 의 "UIArg 페어" 와는 무관 — 단순 MonoBehaviour 분리, 클래스 자체는 같은 namespace `Lair.UI`).
...
| Container Layout | **CHPoolingScrollView 가 자동 처리** (v0.8 — Rule 11 완전 적용으로 v0.4~v0.7 의 raw `VerticalLayoutGroup + ContentSizeFitter` 정책 폐기). CHPoolingScrollView 가 `InitItemTransform` (line 770~) 에서 자식 RectTransform 의 anchor/pivot/anchoredPosition 을 직접 설정 — 별도 Layout 컴포넌트 부착하면 충돌하므로 부착 X |
...
| 섹션 내부 구조 | **`CHPoolingScrollView<BuildIconCell, BuildEntry>` 파생 컴포넌트** (각 섹션마다 1세트, v0.8 — Rule 11 완전 적용으로 v0.5 의 raw ScrollRect+VLG+CSF 정책 교체) |
...
#### 2.8.5 CHPoolingScrollView 구조 단일 결정 값 (v0.8 — Rule 11 완전 적용)
...
| 아이콘 간 간격 | 2px | (유지 — row 의 distinct 아이콘은 항상 0/1 이라 실사용 X) |
...
| **툴팁 본문 구조 (v0.8)** | **`CHPoolingScrollView<BuffLine, AppliedBuff>` 파생 컴포넌트 1세트** (Rule 11 완전 적용 — v0.7 의 raw ScrollRect 구조 교체) | (v0.8 갱신) |
...
| **BuildPanel 섹션 구조 (v0.8)** | **`CHPoolingScrollView<BuildIconCell, BuildEntry>` 파생 컴포넌트** (Rule 11 완전 적용 — v0.5 의 raw ScrollRect+VLG+CSF 정책 교체) | (v0.8 갱신) |
...
| **모달 섹션 구조 (v0.8)** | **`CHPoolingScrollView<BuildModalCardCell, BuildEntry>` 파생 컴포넌트** (각 섹션 1세트, Rule 11 완전 적용 — v0.3 의 raw ScrollRect 예외 철회) | (v0.8 갱신) |
...
**근거**: 둘 다 `CHMUI.ShowUI(EUI.X, arg)` 로 띄우는 단일 인스턴스 popup (Rule 08).
...
### 4.3 데이터 노출 (`BattleController`)

신규 필드 + 이벤트:

```text
//# 종별 적용된 강화 카드 픽 누적. 툴팁 본문 + 셀 상단 아이콘 row 의 source.
private Dictionary<EMonster, List<AppliedBuff>> _typeModifierPicks = new();
public event Action<EMonster> OnTypeModifierChanged;

//# RegisterMonsterTypeBuff 내부에서 _currentCardScope (§4.2) 가 non-null 일 때만 호출:
private void TrackCardPick(EMonster type, EMonsterStatKind stat, CardData source)
{
    ...
    //# 동일 source 카드 누적 픽이면 PickCount++, 신규면 add.
    ...
    //# 동일 종·동일 Stat 의 누적 배율을 _typeModifiers[type].Get(stat) 으로 일괄 갱신.
    //# (종 1 ↔ 카드 1 매핑이므로 동일 종/동일 Stat 의 엔트리는 사실상 1개지만, 데이터 모델
    //# 자체는 향후 1↔다 매핑 확장에 대비해 list 구조 유지.)
    if (_typeModifiers.TryGetValue(type, out var m))
    {
        foreach (var b in list)
            if (b.Stat == stat) b.AggregateMultiplier = m.Get(stat);
    }
}

//# 읽기 전용 노출 — VM 이 스냅샷 채울 때 사용.
...
신규 데이터 구조 (`AppliedBuff` — **`BattleViewModel.cs` 안에 같이 정의**, `Lair.UI` namespace. Rule 10 의 "동일 도메인 단일 파일" 정신 + 기존 `BuildEntry` 가 이미 같은 파일에 있어 일관성 유지):
...
| `Assets/_Lair/Scripts/UI/SpawnerStatusCell.cs` | 1셀 — 색칩·종명·×N·진행바·아이콘 row. **`Bind(SpawnerSnapshot, ISpawnerProgress)` 시그니처로 받아, Progress 는 매 프레임 폴링, 스냅샷 필드는 이벤트 수신 시 교체.** **색칩 visibility 는 `RebindSnapshot` 안에서 `_colorChip.gameObject.SetActive(snapshot.OutputCount < 2)` 로 토글 — N≥2 일 때 숨김 (§2.2.2 v0.5).** | (없음) |
...
| **`Assets/_Lair/Scripts/UI/BuffLine.cs`** (v0.8 신규) | `CHPoolingScrollView<BuffLine, AppliedBuff>` 의 `TItem`. 자식 — `IconLetterRoot Image(원형) + IconLetter CHText 12pt + Badge CHText 10pt + Body CHText 10pt`. **`Bind(AppliedBuff buff, EMonster type, BalanceConfig balance)` 메서드** — 현 `SpawnerStatusTooltip.FormatBuffLine` (현 .cs line 149~205) 의 스탯별 분기 로직을 BuffLine 안으로 **이주** (한 줄 한 줄을 BuffLine 자기책임). namespace `Lair.UI`. | (없음 — 단순 MonoBehaviour) |
...
| `Assets/_Lair/Scripts/Battle/BattleController.cs` | §4.2 — `_currentCardScope` + `ApplyCardEffect(card)` + 3개 call site 치환 + §4.3 `_typeModifierPicks` · `TrackCardPick` · `OnTypeModifierChanged` · `GetAppliedBuffs` |
...
- **v0.7 추가 — `LairSpawnerStatusUIBuilder.cs` 갱신**:
  ...
  - `BuildCellPrefab` (line 83~) 의 모든 내부 위치 / 크기 / 폰트 상수를 §3.2 v0.7 컬럼에 맞춰 갱신:
    ...
    - 아이콘 원 sizeDelta 30×30 / anchoredPosition x = 12
    - 아이콘 글자 폰트 16pt
    - 아이콘 ×N 배지 폰트 14pt / sizeDelta (24, 14)
...
| **BuildPanel / BuildModalPopup / 툴팁 본문 — CHPoolingScrollView 완전 적용** | **MVP 내** (v0.8 — Rule 11 의 `ScrollRect + 수동 풀링 → CHPoolingScrollView` 원칙 완전 적용. v0.5 의 raw ScrollRect 정책 / v0.3 의 모달 예외 / v0.7 의 툴팁 raw ScrollRect 정책 모두 교체) |
| 사운드 hook | MVP 외 (§11.2) |
| 모달 일시정지 결합 | MVP 외 |
...
- **v0.3 (2026-05-27)**: design-reviewer 2차 검토 BLOCKER 0건 + 잔존 권장수정 4건 + 옵션 1건 반영. "재량" / "또는" / "TBD" 잔존 0건 달성.
  - **정정 1 (§2.7.2)**: 모달 스크롤뷰를 `ScrollRect + ContentSizeFitter + VerticalLayoutGroup` 단순 구조로 단일 결정. 모달 카드 셀은 최대 패시브 7장 + 액티브 10장 ≈ 17개로 풀링 이득이 작아 Rule 11 의 CHPoolingScrollView 우선 원칙에서 예외임을 명시. "재량" 표현 제거.
  ...
  - **옵션 5 (§4.3)**: `AppliedBuff` 위치를 `BattleViewModel.cs` 안 단일 결정 (Rule 10 의 동일 도메인 단일 파일 정신, 기존 `BuildEntry` 와 같은 파일). "또는 별도 파일" 표현 제거.
...
- **v0.8 (2026-05-27)**: 사용자가 v0.7 의 모달/툴팁/BuildPanel raw ScrollRect 구조를 보고 **Rule 11 의 `ScrollRect + 수동 풀링 → CHPoolingScrollView` 우선 원칙 완전 적용** 을 결정. 사용자 발화 직접 인용 — **"룰에 chscrollview 쓰라고 안되어있어?"**. 추가로 design-reviewer 의 v0.7 검토 (BLOCKER 3건 + 권장수정 3건) 를 한 패스에 흡수. v0.3~v0.7 의 모든 락 결정은 유지하고, 세 ScrollRect 영역 / BLOCKER 1/2 단정 / 권장수정 3건을 정리.
  - **변경 A (§ 헤더, §1, §2.5.5, §2.7.2, §2.8.2, §2.8.3, §2.8.5, §3.2, §4.6, §4.7, §4.9, §4.11) — Rule 11 완전 적용**: 사용자 결정 인용 후 세 raw ScrollRect 영역을 모두 `CHPoolingScrollView<TItem, TData>` 파생 컴포넌트로 재단정.
    ...
    - **§2.7.2 BuildModalPopup 좌/우 섹션** — `CHPoolingScrollView<BuildModalCardCell, BuildEntry>` 각 섹션 1세트 (v0.3 의 "Rule 11 예외" 결정 철회).
