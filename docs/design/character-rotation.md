...
**위치**: `Assets/_Lair/Scripts/Character/CommonInterface.cs` (Rule 10 — 단일 파일에 추가).
...
**참고 (외부 사용)**: Rule 06 — 외부에서 회전을 다룰 필요가 생기면 `GetComponentInParent<IRotator>()` 또는 `GetComponent<IRotator>()` 로 접근. 구체 클래스 `SimpleRotator` 직접 참조 금지.
