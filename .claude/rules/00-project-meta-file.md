...
UI(화면·프리팹) 를 **생성/배선하는 작업**에 착수하기 전에, 메인은 **먼저 목업을 사용자에게 제시하고 승인을 받은 뒤에만** 실제 UI 배선(UnityMCP 호출·에디터 빌더 실행)을 진행한다. 승인 전 UI 배선을 시작하지 않는다.

- **적용 조건** — UI GameObject/프리팹을 새로 만들거나 위젯을 배선하는 작업(탭·버튼·아이콘·셀·패널 등). 순수 로직/데이터(.cs·SO 값)나 UI 아닌 에셋 작업은 대상 아님.
- **목업 소스** — game-designer 기획서에 UI 목업(예: `.mockups/*.html`)이 있으면 그것을 제시한다. 없으면 **목업을 먼저 만들어** 제시한 뒤 승인을 받는다. 로컬 프리뷰가 필요하면 사용자에게 실행 방법을 안내(예: `! node .mockups/server.js`)한다.
- **승인 후 진행** — 사용자가 목업을 승인하면 그때 UnityMCP/빌더로 배선한다. 사용자가 수정을 요청하면 목업을 고쳐 다시 이 게이트로 돌아온다.
- **자체 분기 금지와의 관계** — 메인이 "이 정도 UI는 목업 없이 바로" 라고 임의 판단하지 않는다. 목업 제시 → 승인이 단일 진실. 결정은 사용자가 한다.
- **파이프라인 위치** — start-develop 계열에서 구현이 UI 배선을 포함하면, 그 배선(MCP/빌더 실행) 직전에 이 게이트가 걸린다. 코드 구현·code-reviewer 는 이 게이트와 무관하게 진행 가능하나, **MCP UI 배선 실행만은 목업 승인 이후**로 미룬다.

### 데이터 설계 — 구조 확인 게이트

**새 JSON 데이터 스키마(row 클래스)를 설계하는 작업**은, 메인이 **먼저 필드명·구조를 표(또는 트리)로 제시하고 사용자 승인을 받은 뒤에만** 실제 구현(row 클래스 작성, gameplay-programmer 위임)에 착수한다. 스키마 설계 자체는 `02-csharp-style.md` §11-1(정규화 절차: 핵심 엔티티 정의 → 선택적/필수 관계 분리 → 복합 키 → 관계 데이터 이름 규칙)을 따른다.

- **구조 표현 형태** — 각 데이터(엔티티/관계 데이터 포함)의 필드명과, 다른 데이터와의 관계(어떤 ID가 무엇을 가리키는지)가 한눈에 보이는 표 또는 트리 다이어그램. 필드 나열만으로는 부족하다 — 어떤 게 엔티티고 어떤 게 관계 데이터인지 구조가 보여야 한다.
- **승인 후 진행** — 승인이 떨어지면 그때 spec/plan에 반영하고 구현에 들어간다. 사용자가 구조 변경(필드 추가/분리/이름 변경 등)을 요청하면 표를 고쳐 다시 이 게이트로 돌아온다.
- **제외 케이스** — 이미 확정된 스키마에 실제 값만 채우는 작업(JSON 데이터 입력 등)은 이 게이트 대상이 아니다. 스키마(필드 구성) 자체가 신설되거나 바뀔 때만 해당.
