...
namespace Lair.Battle
{
    ...
    public class BattleZone : MonoBehaviour
    {
        ...
        //# 영웅 이동 클램프 직사각 반-extent (X/Z 분리). 교전 trigger 와 분리 — 클램프만 축소(기획서 §4.2).
        [SerializeField] private float _clampHalfExtentX = 10f;
        [SerializeField] private float _clampHalfExtentZ = 6f;
        ...
        //# 영웅 SimpleMover 가 매 FixedUpdate next 좌표 클램프에 사용. 교전 trigger 와 분리된 작은 직사각(Center ± X/Z extent)으로.
        //# Y 평면 (X/Z) 만 클램프 — Y 는 입력 그대로 (SimpleMover 가 어차피 0 으로 고정).
        public Vector3 ClampInside(Vector3 worldPos)
        {
            Vector3 center = Center;
            float x = Mathf.Clamp(worldPos.x, center.x - _clampHalfExtentX, center.x + _clampHalfExtentX);
            float z = Mathf.Clamp(worldPos.z, center.z - _clampHalfExtentZ, center.z + _clampHalfExtentZ);
        ...
        //# 영웅 클램프 직사각(Center ± X/Z extent)을 에디터에서 시각화 — 교전 trigger 와 구분(노란색).
        private void OnDrawGizmosSelected()
        {
            Vector3 center = _zoneTrigger != null ? _zoneTrigger.bounds.center : transform.position;
            Gizmos.color = Color.yellow;
            Gizmos.DrawWireCube(center, new Vector3(_clampHalfExtentX * 2f, 1f, _clampHalfExtentZ * 2f));
        }
    }
}
