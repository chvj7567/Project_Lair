...
namespace Lair.Tests.Battle
{
    ...
    public class SpawnerBodyTests
    {
        ...
        //# 머티리얼 배열 생성 (EMonster 순서: 0=Slime, 1=Golem, 2=Orc, 3=Archer, 4=Spider, 5=Bat).
        ...
        private (Spawner spawner, SpawnerBody body, MeshRenderer renderer, Material[] materials)
            CreateSetup(EMonster spawnerOutputType = EMonster.Slime, bool setupBody = true,
                int matCount = 6)
        {
        ...
        [Test]
        public void OnEnable_초기_Slime종으로_머티리얼_적용()
        {
            var (spawner, body, renderer, mats) = CreateSetup(EMonster.Slime);
            ...
            Assert.AreEqual(mats[(int)EMonster.Slime], renderer.sharedMaterial,
                "초기 Slime 종 → mats[0] 적용");
        }

        [Test]
        public void OnEnable_초기_Golem종으로_머티리얼_적용()
        {
            var (spawner, body, renderer, mats) = CreateSetup(EMonster.Golem);
            InvokeOnEnable(spawner);
            InvokeBodyOnEnable(body);

            Assert.AreEqual(mats[(int)EMonster.Golem], renderer.sharedMaterial,
                "초기 Golem 종 → mats[1] 적용");
        }

        //# ===== ReplaceOutput 후 머티리얼 교체 =====

        //# ReplaceOutput(Golem) 시 mats[(int)Golem] 으로 sharedMaterial 교체.
        [Test]
        public void ReplaceOutput_Golem_머티리얼_교체()
        {
            var (spawner, body, renderer, mats) = CreateSetup(EMonster.Slime);
            InvokeOnEnable(spawner);
            InvokeBodyOnEnable(body);

            spawner.ReplaceOutput(EMonster.Golem);

            Assert.AreEqual(mats[(int)EMonster.Golem], renderer.sharedMaterial,
                "ReplaceOutput(Golem) → mats[1] 적용");
        }

        //# Slime → Golem → Orc 연속 교체 시 각 단계에서 정확히 교체.
        [Test]
        public void ReplaceOutput_연속교체_각각_올바른_머티리얼()
        {
            ...
            spawner.ReplaceOutput(EMonster.Golem);
            Assert.AreEqual(mats[(int)EMonster.Golem], renderer.sharedMaterial, "1차 교체 Golem");

            spawner.ReplaceOutput(EMonster.Orc);
            Assert.AreEqual(mats[(int)EMonster.Orc], renderer.sharedMaterial, "2차 교체 Orc");
        }

        //# HandleTypeChanged 직접 호출 — Archer, Bat 경계값.
        [Test]
        public void HandleTypeChanged_Archer_인덱스_3_올바른_머티리얼()
        {
            var (spawner, body, renderer, mats) = CreateSetup(EMonster.Slime);
            InvokeOnEnable(spawner);
            InvokeBodyOnEnable(body);

            InvokeHandleTypeChanged(body, EMonster.Archer);

            Assert.AreEqual(mats[(int)EMonster.Archer], renderer.sharedMaterial,
                "Archer(index=3) 머티리얼 적용");
        }

        [Test]
        public void HandleTypeChanged_Bat_인덱스_5_올바른_머티리얼()
        {
            var (_, body, renderer, mats) = CreateSetup(EMonster.Slime);

            //# HandleTypeChanged 직접 — SpawnerBody 독립 테스트.
            SetPrivate(body, "_renderer", renderer);
            SetPrivate(body, "_materials", mats);
            InvokeHandleTypeChanged(body, EMonster.Bat);

            Assert.AreEqual(mats[(int)EMonster.Bat], renderer.sharedMaterial,
                "Bat(index=5) 머티리얼 적용");
        }
        ...
        {
            var (_, body, _, mats) = CreateSetup(EMonster.Slime, setupBody: false);
            ...
            Assert.DoesNotThrow(() => InvokeHandleTypeChanged(body, EMonster.Slime),
        ...
        {
            var (_, body, _, _) = CreateSetup(EMonster.Slime, setupBody: false);
        ...
        //# _materials 배열이 3개뿐인데 index=5(Bat) — 범위 초과, 교체 안 함, 예외 없음.
        [Test]
        public void _materials_범위초과_인덱스_교체안함_예외없음()
        {
            var (_, body, renderer, _) = CreateSetup(EMonster.Slime, setupBody: false);
            ...
            SetPrivate(body, "_renderer", renderer);
            //# 3개 배열 — index 5(Bat) 는 범위 초과.
            SetPrivate(body, "_materials", MakeMaterials(3));

            Assert.DoesNotThrow(() => InvokeHandleTypeChanged(body, EMonster.Bat),
                "범위 초과 인덱스 → 예외 없음");
        ...
        public void OnDisable_후_ReplaceOutput_머티리얼_변경안됨()
        {
            ...
            //# Slime 머티리얼이 설정된 상태.
            var slimeMat = mats[(int)EMonster.Slime];
