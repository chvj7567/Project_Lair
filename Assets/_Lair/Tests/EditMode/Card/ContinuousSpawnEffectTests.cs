...
namespace Lair.Tests.Card
{
    ...
    public class ContinuousSpawnEffectTests
    {
        ...
        [Test]
        public void SlimeHpBoost_슬라임_Hp_배율_등록()
        {
            var ctx = new FakeBattleContext();
            new SlimeHpBoostEffect().Apply(ctx);

            Assert.AreEqual(1, ctx.Buffs.Count);
            Assert.AreEqual(EMonster.Slime, ctx.Buffs[0].Item1);
            Assert.AreEqual(EMonsterStatKind.Hp, ctx.Buffs[0].Item2);
            Assert.AreEqual(1.5f, ctx.Buffs[0].Item3, 0.0001f);
        }

        //# 정상 — 거미 강화는 SlowFactor 배율 0.75 로 등록 (치환값 아닌 배율, §3.0.1).
        [Test]
        public void SpiderSlowBoost_거미_SlowFactor_배율_0점75_등록()
        {
            var ctx = new FakeBattleContext();
            new SpiderSlowBoostEffect().Apply(ctx);

            Assert.AreEqual(1, ctx.Buffs.Count);
            Assert.AreEqual(EMonster.Spider, ctx.Buffs[0].Item1);
        ...
        [Test]
        public void SpawnSlimes_슬라임_출력증가_호출()
        {
            var ctx = new FakeBattleContext();
            new SpawnSlimesEffect().Apply(ctx);

            Assert.AreEqual(1, ctx.Increments.Count);
            Assert.AreEqual(EMonster.Slime, ctx.Increments[0]);
        }

        //# 정상 — 융합 카드는 from→to 로 ReplaceSpawnerOutput 1회 호출.
        [Test]
        public void ReplaceSlimesToGolem_슬라임에서_골렘_교체_호출()
        {
            var ctx = new FakeBattleContext();
            new ReplaceSlimesToGolemEffect().Apply(ctx);

            Assert.AreEqual(1, ctx.Replaces.Count);
            Assert.AreEqual(EMonster.Slime, ctx.Replaces[0].Item1);
            Assert.AreEqual(EMonster.Golem, ctx.Replaces[0].Item2);
        }
        ...
        [Test]
        public void ReplaceOrcsToArchers_강화나_소환_API는_호출되지_않음()
        {
            var ctx = new FakeBattleContext();
