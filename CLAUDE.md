...
v0.3 의 핵심 추가는 **Unity 클라이언트 ↔ 백엔드 서버 연동** — 익명 기기ID 인증→JWT 로컬 저장, 로컬 MetaProfile 클라우드 백업/복원(충돌 처리), 최단클리어 리더보드 제출/조회.
서버 구현 자체는 **별도 레포(`Project_Lair_Server`, ASP.NET Core+MySQL+Redis) 소관**이며 이 Unity 레포에는 **연동 클라이언트 코드만** 들어온다 (§8). 아트/에셋·사운드는 실제 에셋 사용 가능.
...
- `//` 일반 주석 (Rule 02 §1) — `//#`
- 서버(백엔드) 구현을 이 Unity 레포에서 작성 — 별도 레포 `Project_Lair_Server` 소관. 이 레포에는 연동 클라이언트 코드만 (§8)
- 현 단계(v0.3) 범위 밖 작업 (§8)
