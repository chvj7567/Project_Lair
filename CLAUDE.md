...
@.claude/rules/14-asset-folder-structure.md

## 요약 (Quick Reference)

1. **자동 커밋 금지 / 커밋 메시지 포맷** — `git commit` 실행 X. 단 **관련 파일 `git add`(스테이징)** 후 메시지(안) 전달 (사용자가 바로 커밋만 하면 되도록). 포맷: `# [주제] - 커밋 메시지` (한글, 주제는 `[feat]/[fix]/[refactor]/[chore]/[docs]/[test]/[style]/[perf]/[asset]/[infra]`)
...
9. **공용 Enum 단일 파일** — 여러 시스템에서 참조되는 Enum은 `Assets/_Project/Scripts/Data/CommonEnum.cs` 한 파일에 통합. 카테고리별 Enum 자체는 분리 유지(Rule 08), *파일*만 하나
...
14. **에셋 폴더 구조** — Addressable 게임 에셋은 `Assets/_Project/Art/` 하위 타입별 정리: `Characters`/`FX`/`UI`(프리팹), `Cards`(SO), `Materials`(머티리얼), `Sprites`(이미지). 이동 시 `.meta` 동행으로 GUID 보존. `Resources/` 특수 폴더 금지
